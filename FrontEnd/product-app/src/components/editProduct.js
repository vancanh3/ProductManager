...
export default function EditProduct({ match, location }) {
    ...
    useEffect(() => {
        ...
        GET_PRODUCT_ID(`product`, match.params.id).then(product => {
            ...
            setCategory(product.data.category.categoryId)
        })
        /* GET API CATEGORIES */
    ...
    const EditProduct = (event) => {
        ...
        if (title !== "" && body !== "" && slug !== "" && category > 0 && productId > 0) {
            let product = {
                Title: title,
                Body: body,
                Slug: slug,
            ...
            PUT_EDIT_PRODUCT(`product/updateProduct/${productId}`, product).then(item => {
                if (item.data === 1) {
                    setCheckUpdate(true);
                }
            })
